...
export default function AdminTicketClient({ ticket }: { ticket: Ticket }) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 pb-12">
      <Link href="/admin/suporte" className="btn btn-ghost rounded-2xl">Voltar</Link>
      ...
      <div className="orbit-card-premium p-6">
        ...
        <p className="mt-2 text-sm text-[#64748B]">
          Usuário: {ticket.createdBy.name} · {ticket.createdBy.email} · Status: {ticket.status} · Tipo: {ticket.type} · Prazo:{" "}
          {new Date(ticket.dueAt).toLocaleString("pt-BR")}
        </p>
        ...
        <div className="mt-6 flex gap-2">
          <input className="input h-11 flex-1" value={msg} onChange={(e) => setMsg(e.target.value)} placeholder="Resposta do administrador…" />
