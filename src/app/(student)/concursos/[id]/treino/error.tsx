...
import { RefreshCw, LayoutDashboard, Play } from "lucide-react";

export default function TreinoError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[treino] page error:", error?.message, (error as any)?.digest);
  }, [error]);
      ...
      <div className="mx-auto w-full max-w-[680px]">
        <div className="rounded-[20px] border border-black/[0.08] bg-white p-8 shadow-[0_1px_0_rgba(255,255,255,0.7)_inset,0_18px_50px_rgba(17,24,39,0.06)]">
          <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-50">
            <Play className="h-6 w-6 text-violet-600" strokeWidth={1.5} />
          </div>
          ...
          <p className="mt-2 text-[14px] leading-relaxed text-[#6B7280]">
            Ocorreu um erro inesperado ao abrir a tela de treino. Tente novamente. Se persistir, verifique se seu concurso/cargo/matérias estão corretamente vinculados.
          </p>
